...
//        private const double X_TO_M = 0.0000254;  // ADJUST THIS based on diagnostics
//        private const double Y_TO_M = 0.0000254;   // Confirmed correct by user
//        private const double Z_TO_M = 1.0;        // For Z elevation (already in meters, no conversion)


//        private double MX(double xValue) => xValue * X_TO_M;
//        private double MY(double yValue) => yValue * Y_TO_M;
//        private double MZ(double zValue) => zValue * Z_TO_M;  // Add this
...
//                    System.Diagnostics.Debug.WriteLine($"  Z_TO_M factor: {Z_TO_M}");
//                    System.Diagnostics.Debug.WriteLine($"  Sample elevation: 4.5m → {MZ(4.5)}m (after conversion)");
...
//                // Line 267-270: Use MZ for all Z coordinates
//                string[] pts = new string[4];
//                sapModel.PointObj.AddCartesian(MX(line.StartPoint.X), MY(line.StartPoint.Y), MZ(elevation), ref pts[0], "Global");
//                sapModel.PointObj.AddCartesian(MX(line.EndPoint.X), MY(line.EndPoint.Y), MZ(elevation), ref pts[1], "Global");
//                sapModel.PointObj.AddCartesian(MX(line.EndPoint.X), MY(line.EndPoint.Y), MZ(elevation + floorHeight), ref pts[2], "Global");
//                sapModel.PointObj.AddCartesian(MX(line.StartPoint.X), MY(line.StartPoint.Y), MZ(elevation + floorHeight), ref pts[3], "Global");
...
//                // Line 362-365: Update CreateWallSegmentWithAutoThickness
//                string[] pts = new string[4];
//                sapModel.PointObj.AddCartesian(x1M, y1M, MZ(elevation), ref pts[0], "Global");
//                sapModel.PointObj.AddCartesian(x2M, y2M, MZ(elevation), ref pts[1], "Global");
//                sapModel.PointObj.AddCartesian(x2M, y2M, MZ(elevation + floorHeight), ref pts[2], "Global");
//                sapModel.PointObj.AddCartesian(x1M, y1M, MZ(elevation + floorHeight), ref pts[3], "Global");
//                string area = "";
//                int ret = sapModel.AreaObj.AddByPoint(4, ref pts, ref area, section);
